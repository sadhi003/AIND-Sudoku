...
rows = 'ABCDEFGHI'
cols = '123456789'
cols_rev = cols[::-1]   # reverse the cols
...
# need to apply suggestions  http://www.petercollingridge.co.uk/book/export/html/362
#dig2_units = [[rows[i]+cols_rev[i] for i in range(len(rows))]]
dig2_units = [[r+c for r,c in zip(rows,cols)], [r+c for r,c in zip(rows,cols[::-1])]]  # last expression make it in reverse order
...
def naked_twins(values):
    ...
    for unit in unitlist:
        # make dictionary with twin pairs
        vdict = {}
        for box in unit:
            # identify and put values in dict.
            if len(values[box]) == 2:
...
def eliminate(values):
    ''' eliminate the values which length is only one '''
...
def search(values):
    ...
    if all(len(values[s]) == 1 for s in boxes): 
        return values ## Solved here giving value on each 1
